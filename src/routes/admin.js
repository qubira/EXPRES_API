...
router.get('/tiendas', async (req, res) => {
  const { rows } = await db.query('SELECT id, nombre, categoria, zona, comision_pactada, activo, email, contacto_whatsapp, created_at FROM tiendas ORDER BY created_at DESC');
  res.json(rows);
});

router.post('/tiendas', async (req, res) => {
  try {
    const { nombre, categoria, descripcion, contacto_telefono, contacto_whatsapp, zona, comision_pactada, email, password } = req.body;
    ...
    const { rows } = await db.query(
      ...
      [nombre, categoria, descripcion || null, contacto_telefono || null, contacto_whatsapp || null,
...
router.put('/tiendas/:id', async (req, res) => {
  try {
    const { nombre, categoria, descripcion, contacto_telefono, contacto_whatsapp, zona, comision_pactada, activo } = req.body;
    await db.query(
      `UPDATE tiendas SET nombre=$1, categoria=$2, descripcion=$3, contacto_telefono=$4,
        contacto_whatsapp=$5, zona=$6, comision_pactada=$7, activo=$8 WHERE id=$9`,
      [nombre, categoria, descripcion || null, contacto_telefono || null, contacto_whatsapp || null,
